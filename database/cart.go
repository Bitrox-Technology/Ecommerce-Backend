...
var (
	...
	ErrCantGetItem        = errors.New("unable to get the items from the cart")
	ErrCantBuyItem        = errors.New("can't update the product")
)
...
func BuyItemFromCart(ctx context.Context, userCollection *mongo.Collection, userID string) error {
	...
	orderCart.Payment_Method.COO = true
	...
	var gerusercart []bson.M
	if err = currentResults.All(ctx, &gerusercart); err != nil {
		panic(err)
	}

	var total_price int32

	for _, user_item := range gerusercart {
...
func InstantBuyer(ctx context.Context, prodCollection, userCollection *mongo.Collection, productID primitive.ObjectID, userID string) error {
	...
	orders_detail.Payment_Method.COO = true
